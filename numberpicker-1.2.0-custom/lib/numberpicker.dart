...
class NumberPicker extends StatelessWidget {
  ...
  Widget _integerListView(ThemeData themeData) {
    TextStyle defaultStyle = themeData.textTheme.subtitle1;
  ...
  Widget _integerInfiniteListView(ThemeData themeData) {
    ...
    if (index != null && count != null)  {
      if (index < (count * 0.3).floor()) {
        opacity = 0.2;
      } else if (index > (count / 3).floor() && index < (count / 6).floor()) {
        opacity = 0.1;
      }
    }
    TextStyle defaultStyle = TextStyle(fontSize: 30.0, fontWeight: FontWeight.bold, color: Colors.black.withOpacity(opacity));
    TextStyle selectedStyle = TextStyle(fontSize: 30.0, fontWeight: FontWeight.bold);
